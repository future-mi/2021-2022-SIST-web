...
public class DataBoardDAO {
	...
	public void databoardInsert(DataBoardVO vo) {
		try {
    		getConnection();
    		String sql="INSERT INTO databoard(no,name,subject,content,pwd,group_id,filename,filesize) "
	...
	public boolean databoardDelete(int no,String pwd) {
		...
		try {
			...
			// 2-2. depth : 0 답변(X) → 삭제, 0이상(답변이 있는 상태) → 수정
			// 2-3. depth를 감소
			conn.commit();
		} catch (Exception ex) {
			ex.printStackTrace();
