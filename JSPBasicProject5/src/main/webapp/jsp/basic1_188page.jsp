...
<%--
    ...
    0. JSP의 동작과정
      1) 브라우저(클라이언트)
        ...
        - 브라우저의 주소(URL)란 → 데이터 노출(검색어,페이지요청,상세보기 번호) : GET
      ...
      4) jsp를 찾으면 → jsp를 자바로 변경(.java)
                     ...
                     → 클라이언트 브라우저를 읽어서 출력
      5) 변경(JSP → Java)
        - 웹에서 실행하는 자바파일 → 서블릿(순수하게 자바로만 코딩)
          MVC(Java와 HTML을 분리해서 코딩)
          JSP는 확장성(X), 재사용성(X)
          Java는 확장성(O), 재사용성(O)
          Model : VO, DAO, Manager → 자바로 요청 처리
                 .Java
          View : JSP, 화면에만 출력
          Controller : Java+HTML을 연결 → servlet → 스프링(서블릿)
          → 변경
          (1) import
       
    1. 구분(자바, HTML)
        <% %> : 스크립트릿 → 일반자바코딩
                         (지역변수선언, 메소드호출, 클래스객체생성, 제어문 등)
    ...
    3. 내장객체 (Cookie → 일반클래스)
      1) request : 클라이언트 가 보내주는 요청 데이터가 포함
            - getRemoteAddr() : 클라이언트의 IP(조회수 증가)
                                로그파일을 제작
            - getRequestURL() : 서버정보, 클라이언트 요청정보 포함
                                ?는 제외(?뒤에있는 내용은 request에 첨부)
            - getRequestURI()
            - getContextPath()
        [ ?뒤에있는 데이터 읽기 ]
            - getParameter()  
            - getParameterValues()
            - setCharacterEncoding() : 한글변환(한글인코딩 → 디코딩)
                인코딩(byte[]) → 한글변환(디코딩:String)
      2) response
        - 응답정보 : 브라우저 출력 형태(HTML/XML/JSON)
                   클라이언트에 저장(Cookie)
                   setContentType("text/html")
        - 재전송(화면변경) : redirect()
                        sendRedirect("이동할 파일명") → GET방식
        - 화면변경전 사전에 데이터를 전송 : header
                        setHeader() → 다운로드창
      3) session : 클라이언트의 일부 정보 → 프로젝트에서 사용되는 모든 JSP
                   데이터 공유(id,pwd,name), 장바구니, 예매
        ...
        - setAttribute() : 저장
          ...
          → 클라이언트당 1개만 저장하는 공간(자동생성 ID → getID())
          → 접속자끼리 채팅         
      4) out : 출력버퍼를 관리하는 객체
      ...
      5) application : 서버정보
        ...
        - getRealPath() → 운영체제의 파일 경로명 호환(window, Mac)
        - log : 서버의 동작을 모니터링
      6) pageContext : 내장객체를 관리, 조립식 관련
        - include() → <jsp:include>
    4. 액션태그 : <jsp:~>
      1) 객체생성
            Member m=new Member()
            <jsp:useBean id="m" class="Member"> Member클래스의 m 객체를 생성
      2) 요청값 받기
            → id,name,sex
            Member m=new Member()
      ...
      3) 다른 JSP첨부
            <jsp:include page="파일(동적)"> : 동적
      4) request의 유지(request는 매개변수 → 화면 변경시 초기화)
            <jsp:forward> : 오버라이드
            → 화면이동: 새로운파일
                     기존의 파일에 새로운 파일을 덮어쓴다
    5. 자바빈즈
        - JSP : ~Bean
        - MyBatis : ~DTO
        - Spring : ~VO
